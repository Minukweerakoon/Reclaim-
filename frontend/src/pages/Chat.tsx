...
function Chat() {
    ...
    const handleSend = async () => {
        ...
        // Prepare history without the latest message
        // Note: The backend likely expects history to NOT include the current message in the 'history' array, 
        // but the current message is sent in the 'message' field.
        // We should filter out the message we just added (or use the previous state).
        const history = messages;

        try {
            // If we have attachments, we might need a different API call or strategy.
            // The current ChatRequest interface and chatApi only support text + history.
            // Assuming for now the chat endpoint handles multimodal via separate calls or the backend 
            // integrates them. The original app.js handled this by sending standard messages.
            // If the Python backend's /api/chat/message endpoint doesn't support file uploads directly, 
            // we might strictly strictly text for now in this interface, OR 
            // we need to update the API client if the backend supports multipart/form-data for chat.
            // 
            // Looking at the legacy code, it seems the chat was text-based, but could trigger 
            // validations that returned results. 
            // Let's stick to the text interface defined in `src/api/chat.ts` for now.
            // If attachments are needed, we would need to upload them first or use the /validate endpoint.

            // However, the `ValidationHub` does the heavy lifting for files. 
            // The chat might just be for text interaction.
            // Let's implement text-only chat for now to match the strict interface, 
            // but keep the UI for attachments if we want to expand later.
            ...
            const botMessage: ChatMessage = { role: 'bot', content: response.bot_response };
            ...
            // If the chat triggers a new validation result (simulated or real), we could update it.
            // The current backend response includes `extracted_info`, `intention`, etc.
            // If `extracted_info` is rich enough, it might update our understanding of the reported item.
