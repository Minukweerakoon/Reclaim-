...
export function LoginPage({ onBack }) {
    ...
    return (
        <div className="min-h-screen w-full bg-[#08080f] flex items-center justify-center relative overflow-hidden px-4">
            ...
            {onBack && (
                <button onClick={onBack} className="absolute top-6 left-6 flex items-center gap-2 text-sm text-slate-400 hover:text-white transition-colors z-20">
            ...
            <div className="relative z-10 w-full max-w-sm animate-fade-in">
                <div className="glass-panel rounded-3xl p-8 flex flex-col items-center text-center"
                    ...
                    <p className="text-sm text-slate-400 mb-8 leading-relaxed max-w-xs">
                        Find what's lost. Return what's found.<br />
                    ...
                    <button onClick={handleGoogleSignIn} disabled={loading}
                        ...
                        {loading ? (
                            <><div className="w-4 h-4 border-2 border-slate-300 border-t-slate-600 rounded-full animate-spin" />Signing in…</>
                        ) : (
                            <>
                    ...
                    <div className="flex items-center gap-3 w-full my-5">
                        <div className="flex-1 h-px bg-white/5" /><span className="text-[10px] text-slate-600 uppercase tracking-wider">or</span><div className="flex-1 h-px bg-white/5" />
                    </div>

                    <div className="flex items-center gap-1.5 mt-6 text-[10px] text-slate-600">
                        <Shield className="w-3 h-3" /><span>Your data is encrypted and never shared.</span>
                    </div>
                </div>
