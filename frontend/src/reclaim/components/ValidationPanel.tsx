...
import { Info, TrendingUp, Tag, BarChart2, CheckCircle2, AlertTriangle, XCircle } from 'lucide-react';
...
function CircularProgress({ value, size = 96 }) {
    ...
    const glowColor = value >= 80 ? 'rgba(34,197,94,0.3)' : value >= 50 ? 'rgba(245,158,11,0.3)' : 'rgba(239,68,68,0.3)';
    return (
        <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
            ...
            <div className="absolute flex flex-col items-center">
                ...
                <span className="text-[9px] text-slate-500 mt-0.5 uppercase tracking-wider">score</span>
            </div>
        </div>
    );
}
...
function Tooltip({ text }) {
    ...
    return (
        <div className="relative inline-flex">
            <button onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)} className="text-slate-500 hover:text-slate-300 transition-colors">
...
export function ValidationPanel({ isVisible, confidence, imageTextSimilarity, userCategory, modelCategory, topMatchScore, alphaWeight, entropy, matchesFound }) {
    const alignmentThreshold = 75;
    const alignmentStatus = imageTextSimilarity >= alignmentThreshold + 10 ? 'Aligned'
        : imageTextSimilarity >= alignmentThreshold ? 'Low Alignment' : 'Mismatch';
    ...
    const alignmentColor = alignmentStatus === 'Aligned'
        ? 'bg-green-500/15 text-green-400 border-green-500/20'
        : alignmentStatus === 'Low Alignment'
            ? 'bg-amber-500/15 text-amber-400 border-amber-500/20'
            : 'bg-red-500/15 text-red-400 border-red-500/20';
    const AlignmentIcon = alignmentStatus === 'Aligned' ? CheckCircle2 : alignmentStatus === 'Low Alignment' ? AlertTriangle : XCircle;
        ...
        return (
            <div className="w-72 flex-shrink-0 hidden lg:flex flex-col gap-3 pt-6 pr-4 pb-4 opacity-40">
                <div className="glass-panel rounded-2xl p-5 flex flex-col items-center justify-center gap-2 h-40">
                    ...
                    <p className="text-xs text-slate-500 text-center">Validation panel<br />appears after results</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="w-72 flex-shrink-0 hidden lg:flex flex-col gap-3 pt-6 pr-4 pb-4 overflow-y-auto animate-fade-in">
            ...
            <div className="glass-panel rounded-2xl p-4 border border-white/5">
                <SectionHeader icon={<TrendingUp className="w-3.5 h-3.5" />} title="Match Confidence" />
                <div className="flex items-center gap-4">
                    <CircularProgress value={confidence} />
                    <div className="flex-1">
                        <p className={`text-2xl font-bold ${confidenceColor}`}>{confidence}%</p>
                        <p className="text-xs text-slate-400 mt-0.5">
                            {confidence >= 80 ? 'High confidence match' : confidence >= 50 ? 'Moderate confidence' : 'Low confidence'}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="glass-panel rounded-2xl p-4 border border-white/5">
                <SectionHeader icon={<CheckCircle2 className="w-3.5 h-3.5" />} title="Image–Text Alignment" />
                <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-1.5">
                        <span className="text-lg font-bold text-white">{imageTextSimilarity}%</span>
                        <span className="text-xs text-slate-500">similarity</span>
                    </div>
                    <div className="flex items-center gap-1">
                        <span className={`flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full border ${alignmentColor}`}>
                            <AlignmentIcon className="w-2.5 h-2.5" />{alignmentStatus}
                        </span>
                ...
                <div className="relative h-1.5 bg-white/5 rounded-full overflow-hidden">
                    <div className="h-full rounded-full transition-all duration-700" style={{
                        ...
                        background: alignmentStatus === 'Aligned' ? '#22c55e' : alignmentStatus === 'Low Alignment' ? '#f59e0b' : '#ef4444',
                    }} />
                </div>
            </div>
            ...
            <div className="glass-panel rounded-2xl p-4 border border-white/5">
                <SectionHeader icon={<Tag className="w-3.5 h-3.5" />} title="Category Comparison" />
                <div className="space-y-2">
                    {[
                        ...
                        <div key={label} className={`flex items-center justify-between px-2.5 py-1.5 rounded-lg ${highlight ? 'bg-blue-500/10 border border-blue-500/15' : 'bg-white/[0.03]'}`}>
                            <span className="text-[10px] text-slate-400">{label}</span>
                            <span className={`text-xs ${bold ? 'font-semibold text-white' : 'text-slate-200'}`}>{value}</span>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="glass-panel rounded-2xl p-4 border border-white/5">
                <SectionHeader icon={<BarChart2 className="w-3.5 h-3.5" />} title="Retrieval Analytics" />
                <div className="grid grid-cols-2 gap-2">
                    {[
                        ...
                        { label: 'Matches', value: String(matchesFound ?? 0), sub: 'found' },
                    ].map(({ label, value, sub }) => (
