...
export function ChatInterface({ onResultsReady, user }) {
    ...
    const [formData, setFormData] = useState({ status: null, location: '', time: '', description: '', category: '' });
    ...
    const triggerSearch = async (finalFormData) => {
        try {
            ...
            setIsTyping(true);
            if (finalFormData.status === 'lost') addMessage('Scanning database with AI…', 'ai');
            else addMessage('Registering found item…', 'ai');
                ...
                if (response?.results && response.results.length > 0) {
                    ...
                    addMessage('', 'ai', 'results', {
                        results: response.results.slice(0, 4).map((r, index) => ({
                            id: r.id, image_url: r.image_url, final_category: r.category,
                            score: r.score, location: r.location || 'Reported location',
                            reported_time: null, isBestMatch: index === 0,
                        })),
                    });
                } else {
    ...
    const handleSend = () => {
        ...
        if (currentStep === 'IMAGE' && selectedFile) {
            addMessage('Photo attached', 'user', 'image', { imageUrl: previewUrl });
        } else {
        ...
        switch (currentStep) {
            case 'GREETING': {
                ...
                addMessage(`Got it — you're reporting a ${status} item. Please upload a photo of the item.`, 'ai');
                break;
            }
            case 'IMAGE': {
                if (!selectedFile) { addMessage('Please upload an image before continuing.', 'ai'); return; }
            ...
            case 'TIME': {
                ...
                addMessage('Please describe the item in detail (color, brand, unique marks).', 'ai');
                break;
            }
            case 'DESCRIPTION': {
                setFormData((prev) => ({ ...prev, description: value }));
                setCurrentStep('CATEGORY');
                addMessage('What category best fits this item? (e.g. wallet, phone, keys)', 'ai');
                break;
            }
            case 'CATEGORY': {
        ...
        <div className="flex flex-col h-[calc(100vh-64px)] max-w-2xl mx-auto relative">
                    ...
                    <ChatBubble key={msg.id} message={msg.text} sender={msg.sender} timestamp={msg.timestamp}>
                        ...
                        {msg.type === 'results' && msg.results && (
                            <div className="mt-4 space-y-3 w-full">
                                {msg.results.map((result) => <MatchResultCard key={result.id} {...result} />)}
                            </div>
                        )}
                    </ChatBubble>
                ))}
                {isTyping && <ChatBubble sender="ai" isTyping />}
            ...
            <div className="p-4 glass-panel border-t border-white/5 mx-4 mb-4 rounded-2xl">
                ...
                <div className="flex items-center gap-3">
                    ...
                    />
                    <button onClick={handleSend} className="p-3 rounded-full bg-indigo-600 hover:bg-indigo-500 text-white transition-colors">
