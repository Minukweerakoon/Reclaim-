...
export function ChatBubble({ message, sender, timestamp, isTyping, children }) {
  ...
  // Render the animated typing indicator if the AI is "thinking"
  if (isTyping) {
    return (
  ...
  return (
    <div
      className={`flex w-full mb-6 animate-fade-in ${isAI ? 'justify-start' : 'justify-end'}`}
    >
      <div
        className={`flex items-end gap-3 max-w-[85%] md:max-w-[75%] ${isAI ? 'flex-row' : 'flex-row-reverse'}`}
      >
        {/* Avatar Icon */}
        <div
          className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
            isAI ? 'bg-cyan-500/10 border border-cyan-500/20' : 'bg-indigo-500/10 border border-indigo-500/20'
          }`}
        >
          {isAI ? (
            <Bot className="w-4 h-4 text-cyan-400" />
        ...
        <div className="flex flex-col gap-1">
          {isAI && (
            <span className="text-[10px] font-medium text-cyan-400 ml-1">
              Reclaim AI
            </span>
          )}
          <div
            className={`px-5 py-3.5 shadow-lg backdrop-blur-sm ${
              isAI 
          ...
          >
            ...
            {/* children allows us to inject images or result cards inside the bubble */}
            {children}
          </div>
          ...
          {timestamp && (
            <span
              className={`text-[10px] text-slate-500 ${isAI ? 'text-left ml-1' : 'text-right mr-1'}`}
            >
              {timestamp}
            </span>
          )}
