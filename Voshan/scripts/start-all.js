...
/**
 * Run Voshan Node backend + ML service together for production/runtime environments.
...
if (!fs.existsSync(mlDir)) {
  console.error('[start-all] Missing ml-service directory:', mlDir);
  process.exit(1);
}

console.log('[start-all] Starting Voshan backend + ML service...');
console.log(`[start-all] Python: ${pythonCmd}`);
...
backend.on('exit', (code, signal) => {
  if (!shuttingDown) {
    console.error(`[start-all] Backend exited (code=${code}, signal=${signal}). Stopping all services.`);
    shutdown(code || 1);
  }
});
...
ml.on('exit', (code, signal) => {
  if (!shuttingDown) {
    console.error(`[start-all] ML service exited (code=${code}, signal=${signal}). Stopping all services.`);
    shutdown(code || 1);
  }
});
